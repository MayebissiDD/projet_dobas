...
export default function AgentNotifications() {
        ...
        setNotifications((prev) => [
          {
            ...
            message: notification.data?.message || notification.message || 'Notification',
  ...
  const markAllAsRead = () => {
    ...
    Inertia.post(route('agent.notifications.markAllAsRead')); // backend
  };
  ...
  const markAsRead = (id) => {
    ...
    Inertia.post(route('agent.notifications.markAsRead', { id }));
  };
  
  // Supprimer une notification
  const deleteNotification = (id) => {
    setNotifications((prev) => prev.filter((n) => n.id !== id));
    Inertia.delete(route('agent.notifications.destroy', { id }));
  };
    ...
    <AgentLayout>
      <div className="py-10 px-6 md:px-20 bg-gradient-to-br from-white via-zinc-50 to-zinc-100 dark:from-zinc-900 dark:to-zinc-800 text-zinc-900 dark:text-white">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-6 flex justify-between items-center">
          ...
          <Button onClick={markAllAsRead} variant="secondary" disabled={notifications.every(n => n.read)}>
        ...
        {Object.entries(groupedByDate).length > 0 ? (
                ...
                {notifs.map((notif) => (
                  <motion.div
                    ...
                    className={cn(
                      ...
                      notif.read ? "bg-zinc-100 dark:bg-zinc-800" : "bg-blue-50 dark:bg-zinc-700",
          ...
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-zinc-900">Aucune notification</h3>
